...
class CRUDBIFolder(CRUDBase[BIFolder, BIFolderCreate, BIFolderUpdate]):
    ...
    def get_folders_by_agent_id(
        ...
        *,
        ...
        agent_folder_list: List[AgentFolders] = []
        
        folder_search: SearchQueryModel = SearchQueryModel(
            db,
            search_column=[
                BIFolder.idx,
                BIFolder.agent_id_path_hash,
                # UserBIFolder.agent_user_id,
            ...
            #     BIFolder.agent_id == agent_id
            # ])
            # ],
            filters=[BIFolder.agent_id == agent_id]
        ...
        #     ))
        return cast(List[BIFolder],self.get(folder_search))

    def get_folders_by_agent_user_id(
        self,
        db: Session,
        *,
        agent_user_id
        ...
        report_search: SearchQueryModel = SearchQueryModel(
            ...
            relationship=[
                BIFolder.idx == UserBIFolder.bi_folder_id,
                UserBIFolder.agent_user_id == agent_user_id
            ])
            ]
        )
