...
class CRUDSyncLog(CRUDBase[SyncLog, SyncLogCreate, SyncLogUpdate]):
    def create_log(self,db: Session,user_id: int,sync_log_create: SyncLogCreate) -> int:
        return self.create(
            db,
            user_id=user_id,
    ...
    def update_log(self,db: Session,user_id: int,idx: int,sync_log_update: SyncLogUpdate) -> None:
        self.update(
            db=db,
            user_id=user_id,
