...
from app.schemas.sync_reports import ExistingFolder
from app.schemas.sync_reports import ExistingReport
from app.schemas.sync_reports import FolderReportCountUpdate
from app.schemas.sync_reports import FolderUserDetails
from app.schemas.sync_reports import IncomingFolder
from app.schemas.sync_reports import IncomingReport
from app.schemas.sync_reports import UserDetail


class SyncReports():
    
    def __init__(self, db:Session,sync_id: int,agent_instance_id: int,user_id: int) -> None:
        ...
        self.agent_instance_user_id_map: dict[int: int] = {} # agent_instance_user_id: user_id
        self.agent_instance_user_id_batch_list: list[int] = [] # list of agent_instance_user_id of the batch
        
        self.incoming_report_map: dict[str, IncomingReport] = {} # report guid: IncomingReport
        self.incoming_folder_map: dict[str, IncomingFolder] = {} # folder guid: IncomingFolder
        self.existing_report_map: dict[str, ExistingReport] = {} # report guid: ExistingReport
        self.existing_folder_map: dict[str, ExistingFolder] = {} # folder guid: ExistingFolder
        ...
        self.folders_to_be_inserted: dict[int,List[str]] = {} # depth: list of folder guid of the depth
        ...
        self.sync_reports_log_to_be_inserted: dict[int, dict] = {} # report id: sync_reports_log details
        ...
        self.sync_reports_log_to_be_updated: dict[int, dict] = {} # report id: user_id_list to be updated 
        ...
        self.report_user_mapping_to_be_deleted: List[int] = [] # list of user_bi_report id's
        self.folder_user_mapping_to_be_deleted: List[int] = []  # list of user_folder id's
        self.user_report_item_guid_to_be_deleted: List[str] = [] # list of guid used to delete report reference in other tables
    ...
    def generate_incoming_report_folder_aggregate(self,*, incoming_report_details: List[Tuple[int,dict]]) -> None:
        ...
        """
        logger.debug(f"Starting to generate incoming report and folder aggregate for the sync_id {self.sync_id}")
        ...
        for agent_instance_user_id,incoming_reports in incoming_report_details:
            ...
            for incoming_report in incoming_reports:
                ...
                incoming_report: IncomingReport = incoming_report_map.get(incoming_report_guid)
                # Creating report aggregate(incoming_report_map) based on report guid    
                if  incoming_report :
                    incoming_report.agent_instance_users.append(agent_instance_user_id)
                else:
                ...
                for index, folder_name in enumerate(reversed(report_folder_path_list)):
                    ...
                    folder_path: str = codes.path_delimiter.join(report_folder_path_list[:depth]) + codes.path_delimiter
                    folder_guid: str = sha1( "_".join([str(self.agent_instance_id), folder_path ]).encode()).hexdigest()
                    
                    incoming_folder: IncomingFolder = incoming_folder_map.get(folder_guid)
                    if not incoming_folder:
                        ...
                        if folder_guid not in added_folder_guid:
                            incoming_folder.agent_instance_users.append(agent_instance_user_id) 
                        if index == 0:
                            incoming_folder.user_report_count_map[agent_instance_user_id] = incoming_folder.user_report_count_map.get(agent_instance_user_id,0)+1 
                        else:
                            incoming_folder.user_report_count_map[agent_instance_user_id] = 0        
        ...
        logger.debug(f"Incoming report and folder aggregate generated successfully for the sync_id {self.sync_id}")
        

    def generate_existing_report_aggregate(self) -> None:
        """This method is used to construct existing report aggregate 
        """
        logger.debug(f"Starting to generate existing report aggregate for the sync_id {self.sync_id}")
        existing_report_map: dict[str, ExistingReport] = {}
        logger.debug(f"Getting existing reports from db for the sync_id {self.sync_id}...")
        existing_report_list: List[BIReportDetails] = bi_report.get_reports(self.db,agent_instance_id=self.agent_instance_id,agent_instance_user_id_list=self.agent_instance_user_id_batch_list)
        ...
        self.check_running_status()
        logger.debug(f"Existing report aggregate generated successfully for the sync_id {self.sync_id}")

    def generate_existing_folder_aggregate(self) -> None:
        """This method is used to construct existing folder aggregate 
        """
        logger.debug(f"Starting to generate existing folder aggregate for the sync_id {self.sync_id}")
        existing_folder_map: dict[str, ExistingFolder] = {}
        existing_folder_list: List[BIFolderDetails] = bi_folder.get_folders(self.db,agent_instance_id=self.agent_instance_id,agent_instance_user_id_list=self.agent_instance_user_id_batch_list)    
        logger.debug(f"Getting existing folders from db for the sync_id {self.sync_id}...")
        ...
        if not self.is_admin_sync:
            self.update_progress(5,is_increment=True)
        self.check_running_status()
        logger.debug(f"Existing folder aggregate generated successfully for the sync_id {self.sync_id}")
    ...
    def add_folder_user_mapping(self,incoming_folder: IncomingFolder,existing_folder: Optional[ExistingFolder]=None) -> None:
    ...
    def add_report_user_mapping(self,*, incoming_report: IncomingReport, existing_report: Optional[ExistingReport]=None) -> None:
    ...
    def add_sync_report_log(self,agent_instance_user_id: int,bi_report_id: int,report_sync_status: int)  -> None:
        ...
        sync_report_log_id: int = self.report_id_log_id_map.get(bi_report_id)
    ...
    def update_folder_user_report_count(self, incoming_folder: IncomingFolder,existing_folder: ExistingFolder):
        ...
        for agent_instance_user_id,user_detail in existing_folder.agent_instance_user_details.items():
            existing_folder_report_count_set.add((agent_instance_user_id,user_detail.bi_report_count))            
        ...
        folder_report_count_update: set = existing_folder_report_count_set - set(incoming_folder.user_report_count_map.items())
    ...
    def remove_user_report_item_mappings(self,agent_instance_user_id: int, bi_report_id: int) -> None:
    ...
    def remove_report_user_mapping(self,  existing_report: ExistingReport, incoming_report: Optional[IncomingReport] =None) -> None:
    ...
    def remove_folder_user_mapping(self, existing_folder: ExistingFolder, incoming_folder: Optional[IncomingFolder] = None) -> None:
        ...
        for agent_instance_user_id in folder_users_to_be_deleted:
            user_bi_folder_id: int = existing_folder.agent_instance_user_details[agent_instance_user_id].user_bi_folder_id
    ...
    def insert_folders_db(self) -> dict[str,int]:
        ...
        if self.folders_to_be_inserted:
            ...
            logger.debug(f"Enabling folders in deleted state for the sync_id {self.sync_id}...")
            ...
            logger.debug(f"Enabled deleted folders successfully for the sync_id {self.sync_id}")
            ...
            for depth in sorted(self.folders_to_be_inserted):
                ...
                for folder_guid in self.folders_to_be_inserted[depth]:
                    if folder_guid not in folder_guid_id_map:
                        ...
                        incoming_folder: IncomingFolder =self.incoming_folder_map[folder_guid]
                        source_folder_path: str = incoming_folder.path[:-len(f'{incoming_folder.name}{codes.path_delimiter}')]
                        ...
                        batch_folders_to_be_inserted.append({"name":incoming_folder.name,"source_folder_id":source_folder_id,"path":incoming_folder.path,"agent_instance_id":incoming_folder.agent_instance_id,"depth":incoming_folder.depth,
                        "guid":incoming_folder.guid,"created_at": created_at,"updated_at": created_at,"created_by":self.user_id,"updated_by":self.user_id})
                        folders_to_be_inserted_guid_set.add(incoming_folder.guid)
                        
                if batch_folders_to_be_inserted:    
                    bi_folder.insert_folders(self.db,folder_list=batch_folders_to_be_inserted)
                    # Fetching inserted folders id's 
                    inserted_folders: List[BIFolder] = bi_folder.get_folders_by_creation_time(self.db,created_at=created_at, depth=depth,agent_instance_id=self.agent_instance_id, created_by = self.user_id)
                    ...
                    integrity_error_folders_guid: list[str] = list(folders_to_be_inserted_guid_set-inserted_folders_guid_set)
                    if integrity_error_folders_guid:  
                        folders = bi_folder.get_folders_by_guid(self.db,agent_instance_id=self.agent_instance_id,folder_guid_list=integrity_error_folders_guid)
                        for folder in folders:
                            folder_guid_id_map[folder.guid] = folder.idx
                            inserted_folders_guid_set.add(folder.guid)
            
            logger.debug(f"Inserted {len(self.folders_to_be_inserted)} folders successfully for the sync_id {self.sync_id}...")
    ...
    def insert_reports_db(self, folder_guid_id_map: dict[str,int]) -> dict[str,int]:
        ...
        if self.reports_to_be_inserted:
            ...
            bi_report_list: List[BIReport] = bi_report.get_reports_by_guid_list(self.db,agent_instance_id=self.agent_instance_id,report_guid_list=self.reports_to_be_inserted)
            ...
            logger.debug(f"Enabling reports in deleted state for the sync_id {self.sync_id}...")
            ...
            for report_guid in self.reports_to_be_inserted:
                if report_guid not in report_guid_id_map:
                    incoming_report: IncomingReport =self.incoming_report_map[report_guid]
            ...
            if report_data_list:
                ...
                inserted_reports: List[BIReport] = bi_report.get_reports_by_creation_time(self.db,created_at=created_at, agent_instance_id=self.agent_instance_id, created_by = self.user_id)
                ...
                integrity_error_reports_guid: list[str] = list(reports_to_be_inserted_guid_set-inserted_reports_guid_set)
                # Fetching reports which are not inserted by this sync due to integrity error
                if integrity_error_reports_guid:  
                        reports = bi_report.get_reports_by_guid_list(self.db,agent_instance_id=self.agent_instance_id,report_guid_list=integrity_error_reports_guid)
                        for report in reports:
                            report_guid_id_map[report.guid] = report.idx
            logger.debug(f"Inserted {len(self.reports_to_be_inserted)} reports successfully for the sync_id {self.sync_id}...")
    ...
    def insert_folder_users_db(self, folder_guid_id_map: dict[str,int] ):
        ...
        if self.folder_user_mapping_to_be_inserted:
            logger.debug(f"Inserting folder user mappings for the sync_id {self.sync_id}...")
            ...
            logger.debug(f"Inserted folder user mappings successfully for the sync_id {self.sync_id}...")
    ...
    def insert_report_users_db(self, report_guid_id_map: dict[str,int] ):        
        ...
        if  self.report_user_mapping_to_be_inserted:
            logger.debug(f"Inserting report user mappings for the sync_id {self.sync_id}...")
            ...
            for agent_instance_user_id,report_guid_set in self.report_user_mapping_to_be_inserted.items():
                for report_guid in report_guid_set:      
                    ...
                    self.add_sync_report_log(
                        agent_instance_user_id = agent_instance_user_id,
                        bi_report_id = bi_report_id,
                        report_sync_status= codes.REPORT_SYNC_STATUS["granted"] if existing_report_obj else codes.REPORT_SYNC_STATUS["added"]
                    )

            if insert_report_users:
                user_bi_report.insert_report_users(self.db,report_user_list=insert_report_users)
            logger.debug(f"Inserted report user mappings successfully for the sync_id {self.sync_id}...")
    ...
    def insert_sync_report_logs_db(self):
        ...
        if self.sync_reports_log_to_be_inserted or self.sync_reports_log_to_be_updated:
            logger.debug(f"Inserting/Updating sync reports log for the sync_id {self.sync_id}...")
            created_at: int = codes.DEFAULT_TIME()
            if self.sync_reports_log_to_be_inserted:
                insert_sync_reports_log_list = list(self.sync_reports_log_to_be_inserted.values())
                ...
                sync_reports_log.insert_sync_reports_log(self.db,insert_sync_reports_log_list)
                sync_report_log_list: List[SyncReportsLog] = sync_reports_log.get_log_by_creation_time(self.db,created_at=created_at,
                    agent_instance_id=self.agent_instance_id,sync_id=self.sync_id
                    ) 
                ...
                logger.debug(f"Inserted {len(self.sync_reports_log_to_be_inserted)} sync reports log for the sync_id {self.sync_id}...")
            ...
            if self.sync_reports_log_to_be_updated:
                ...
                logger.debug(f"Updated {len(self.sync_reports_log_to_be_updated)} sync reports log for the sync_id {self.sync_id}...")
    ...
    def update_folder_user_report_count_db(self):
        ...
        if self.folder_user_report_count_update:
            logger.debug(f"Updating folder user's report counts for the sync_id {self.sync_id}...")
            ...
            user_bi_folder.update_folder_users(self.db,folder_user_list=folder_user_report_count)
            logger.debug(f"Updated {len(self.folder_user_report_count_update)} folder user's report counts for the sync_id {self.sync_id}")
    ...
    def update_agent_instance_report_count_db(self):
        """This method is used to update agent_instance bi_report_count
            in agent_instance table
        """
        report_count: int = bi_report.get_reports_count(self.db,agent_instance_id=self.agent_instance_id)
        agent_instance.update_bi_report_count(
            self.db,agent_instance_id=self.agent_instance_id,bi_report_count=report_count,
    ...
    def delete_folders_db(self, folders_to_be_deleted: List[int]):
        ...
        if folders_to_be_deleted:
            ...
            bi_folder.delete_folders(self.db,batch_folders_to_be_inserted=delete_folder_list)
            logger.debug(f"Deleted {len(folders_to_be_deleted)} folders for the sync_id {self.sync_id}")
    ...
    def delete_reports_db(self, reports_to_be_deleted: List[int]):
        ...
        if reports_to_be_deleted:
            ...
            logger.debug(f"Deleted {len(reports_to_be_deleted)} reports for the sync_id {self.sync_id}")
    ...
    def delete_folder_users_db(self):
        ...
        user_bi_folder.delete_folder_users(self.db,folder_user_list=delete_folder_user_list)
        logger.debug(f"Deleted {len(self.folder_user_mapping_to_be_deleted)} folder user mapping for the sync_id {self.sync_id}")
    ...
    def delete_report_users_db(self):
        ...
        user_bi_report.delete_report_users(self.db,report_user_list=delete_report_user_list)
        logger.debug(f"Deleted {len(self.report_user_mapping_to_be_deleted)} report user mapping for the sync_id {self.sync_id}")
    ...
    def delete_user_report_item_mappings(self):
        ...
        logger.debug(f"Deleteing user_report_item_mappings for the sync_id {self.sync_id}...")
        start: int = 0
        ...
        logger.debug(f"Deleted user_report_item_mappings for the sync_id {self.sync_id}")
    ...
    def sync_agent_instance_user_reports(self):
        ...
        self.generate_incoming_report_folder_aggregate(incoming_report_details=get_reports(
            agent_instance_user_id_list=self.agent_instance_user_id_batch_list,bi_report_count=1000)
        ) 
        ...
        print("insert folder users:",len(self.folder_user_mapping_to_be_inserted.get(1,[])),len(self.folder_user_mapping_to_be_inserted.get(1,[])),len(set(self.folder_user_mapping_to_be_inserted.get(1,[]))))
    ...
    def sync_agent_instance_reports(self):
        ...
        self.agent_instance_user_id_map: dict[int, int] = agent_instance_user.get_agent_instance_user_list(self.db, agent_instance_id =self.agent_instance_id)
        agent_instance_user_id_list: List[int] = list(self.agent_instance_user_id_map.keys())
        start: int = 0
        ...
        while start<end:
            ...
            logger.debug(f"Started report for users from {start} to {limit} of sync_id {self.sync_id}...")
    ...
    def start(self,user_detail: Optional[UserDetail] = None):
        ...
        try:
            ...
            if self.is_admin_sync:
                ...
                self.agent_instance_user_id_map = {user_detail.agent_instance_user_id: user_detail.user_id}
                self.agent_instance_user_id_batch_list: List[int] = [user_detail.agent_instance_user_id]
                self.sync_agent_instance_user_reports()
                reports_to_be_deleted: List[int] = bi_report.get_reports_to_be_deleted(db=self.db,agent_instance_id=self.agent_instance_id,
                    report_id_list=self.batch_reports_to_be_deleted
                )
                folders_to_be_deleted: List[int] = bi_folder.get_folders_to_be_deleted(db=self.db,agent_instance_id=self.agent_instance_id,
                    folder_id_list=self.batch_folders_to_be_deleted
                )
            ...
            logger.debug(f"Report sync of sync_id {self.sync_id} completed successfully")
    ...
    def update_progress(self, progress: int, is_increment=False):
        ...
        sync_log.update_log(
            ...
            idx=self.sync_id,
            sync_log_update=SyncLogUpdate(
                progress = max(100,self.progress)
            )
        )
    ...
    def check_running_status(self) -> None:
        ...
        if status == codes.SYNC_STATUS["started"]:
            return
        raise StopSyncException(message=f"Report Sync of sync id {self.sync_id} is interrupted..")